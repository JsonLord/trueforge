...
function vectorsValue(value, expectedCount) {
  ...
  if (!Array.isArray(vectors) || vectors.length !== expectedCount) {
    throw new Error('invalid vector count');
  }
  let dimension;
...
export function createNeedleClient({ url = 'http://127.0.0.1:8792', timeoutMs = 3000, fetchImpl = fetch } = {}) {
  ...
  async function post(path, body) {
    ...
    if (!response.ok) {
      throw new Error(`Needle service returned HTTP ${response.status}`);
    }
  ...
  return {
    ...
    async extractToolArguments({ query, tool }) {
      ...
      if (confidence === null) {
        throw new Error('Needle extraction did not provide native confidence');
      }
